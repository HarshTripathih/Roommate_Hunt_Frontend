...
export const login = (loginComp, loginUser) => {
    ...
    const request = new Request("/base/api/users/login", {
        method: "post",
...
export const logout = (logoutUser) => {
    const url = "/base/api/users/logout"
    fetch(url)
        .then(res => {
            if (res.status === 200){
                logoutUser()
...
export const registerUser = (registerComp, e) => {
    ...
    data.append('phone', registerComp.state.phone)
    data.append('aboutMe', registerComp.state.aboutMe)
    data.append('image', e.target.image.files[0])

    const request = new Request("/base/api/users/new", {
        method: "post",
        body: data,
...
export const reportUserByID = (userid) => {
    ...
    const request = new Request(`/base/api/users/${userid}/report`, {
        method: "post",
    });
    fetch(request)
        .then(res => {
...
export const editProfileInfo = (editProfileComp, photo) => {
    ...
    const url = `/base/api/users/${editProfileComp.state.userid}`;
...
export const editUserPhoto = (e,userid) => {
    // the URL for the request
    const url = `/base/api/users/${userid}/img`;
...
export const editUserPassword = (editPassComp) => {
    // the URL for the request
    const url = `/base/api/users/${editPassComp.state.userid}/password`;
